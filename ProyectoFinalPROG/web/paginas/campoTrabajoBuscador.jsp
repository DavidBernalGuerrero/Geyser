...
<html>
  ...
  <body>
    ...
    <%
      ...
      if (request.getParameter("columna") == null) {
        consulta = "SELECT s.ID_SERIE, r.VISTO, s.TITULO, s.NO_CAPITULOS, s.VALORACION  "
                ...
                + "WHERE u.ID_USUARIO = " + session.getAttribute("idUsuario") + " AND s.TITULO LIKE '%"
                + request.getParameter("buscador") + "%'";
      } else {
        consulta = "SELECT s.ID_SERIE, r.VISTO, s.TITULO, s.NO_CAPITULOS,  "
                + "FROM Series s RIGHT JOIN Registros r ON s.ID_SERIE=r.FK_SERIE LEFT JOIN "
                + "Usuarios u ON u.ID_USUARIO=r.FK_USUARIO "
                + "WHERE u.ID_USUARIO = " + session.getAttribute("idUsuario") + " AND s.TITULO LIKE '%"
                + request.getParameter("buscador") + "%' ORDER BY " + request.getParameter("columna");
      }
    ...
    <footer>
      <div id="buscador" style="float: left">
 ...
 <a style="float: left; margin: 2.5em 0;" class="btn btn-danger" href="campoTrabajo.jsp" role="button">Quitar Filtro</a>            
    </footer>
  </body>
</html>
